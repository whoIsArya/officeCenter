...
<template>
  <div>
    <div class="search">
      ...
      <div>
        <label>使用日期范围：</label>
        <DatePicker :value="searchDate" @on-change="handleSearchDateChange" :clearable="false" format="yyyy/MM/dd" type="daterange" placement="bottom-end" placeholder="选择日期范围"
    ...
    <div class="oper-group">
      ...
      <Button type="primary" @click="handleMeeting('add')">新增申请</Button>
      <Button type="primary" @click="handleMeeting('submit')">提交申请</Button>
    </div>
    ...
    <Modal v-model="myModal" :title="modalTitle" :mask-closable="false">
      <div class="my-modal">
        <div>
          ...
          <div class="form-item">
            <span>使用日期：</span>
            <DatePicker @on-change="handleDateChange" :readonly="isReadOnly" :disabled="isDisabled" :value="useDate" format="yyyy/MM/dd" type="date" placement="bottom-end" style="width: 300px"></DatePicker>
            <b>*</b>
          </div>
          <div class="form-item">
            <span>使用时间范围：</span>
  ...
  export default {
    ...
    methods: {
      ...
      handleMeeting: function (str, row) {
        ...
        this.modalTitle = str === 'add' ? '会议申请-增加' : str === 'edit' ? '会议申请-修改' : '会议申请-查看';
        this.myModal = true;
