...
<template>
  <div>
    ...
    <div class="oper-group">
      ...
      <Button type="primary" @click="handleCarApply('add')">新增申请</Button>
    ...
    methods: {
      ...
      submitApply:function(){
        if(!this.selectArr.length){
          this.$Message.error('请先选择要申请的数据');
          return
        }
        let applyArr = this.selectArr.map(function (item) {
          return {
            Id: item.Id,
            Status: 0
          }
        });
        this.$http.post('Drawout/UpdListStu',applyArr).then((res)=>{
          if(res.status === 200 && res.data>0){
      ...
      handleCarApply: function(str,row){
        ...
        this.myModal = true;
        this.modalTitle = str === 'add' ? '车辆申请-增加' : str === 'check' ? '车辆申请-查看': str === 'del'? '车辆申请-删除':'车辆申请-编辑';
        if(row){
          this.id = row.Id;
