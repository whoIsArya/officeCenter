  ...
  export default {
    ...
    methods: {
      ...
      handleCarApply: function(str,row){
        ...
        this.isDisabled = str === 'check' || str === 'plan';
      ...
      submit: function(){
        ...
        if(this.currentModal === 'plan'){
          let self = this;
          let carNameArr = self.planCarList.filter(function (item) {
            return item.Id === self.selectPlanCar
          })
          let carDriverArr = self.planDriverList.filter(function (item) {
            return item.Id === self.selectPlanDriver
          })
          let paramObj = {
            Id: this.id,
            CarName: carNameArr[0].Name,
            DriverId: carDriverArr[0].Name,
            Status:4
          }
          this.$http.post('Drawout/UpdList',paramObj).then((res)=>{
            if(res.status === 200 && res.data>0){
