        ...
        <div class="floor">
          ...
          <div class="right-floor">
            ...
            <div>
              ...
              <Button type="primary" @click="deleteNote(currentData.Id)">删除</Button>
            </div>
          </div>
        </div>
        <div class="floor" v-for="(item,index) in followDataList">
          ...
          <div class="right-floor">
            ...
            <div>
              ...
              <Button type="primary" @click="deleteFollow(item.Id)">删除</Button>
            </div>
          </div>
        </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      deleteCollect:function(obj){
        this.$http.post('Collection/DelList',[obj.Id]).then((res)=>{
      ...
      deleteFollow: function(id){
        let arr = [id];
        this.$http.post('FollowPoster/DelList',arr).then((res)=>{
          if(res.status === 200 && res.data > 0){
            this.$Message.success('删除成功');
          }else{
            this.$Message.error('删除失败');
      ...
      getData:function () {
        ...
        this.paramsData.Name = 'me';
        this.paramsData.Sysid = 0
        this.$http.get('Collection/GetList',{params:this.paramsData}).then((res)=>{
          if(res.status === 200 && res.data.data){
